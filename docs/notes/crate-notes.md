...
`english`（英文模式候选）/ `symbol`（符号候选：按**敲的输入码**查 `assets/symbol/symbol-zh.tsv`，`duigou` → ✔，插在第 2 位；与 emoji 按候选词文本查、紧跟那个词不同——名字不在词库里的符号只有前者查得到。两样合起来最多占第一页 4 格，`[general] extras` 四选一控制出不出）/ `engine`（`query::EnglishTail`：句末英文词并入整句，`woxiangxuehaorust` → 我想学好rust，尾段也像拼音时按分数与拼音读法比）。
