...
> **这一页记的是一版没进 main 的尝试：把 Engine 直接放进 IBus 引擎进程。** 现行方案是 Rust Server + 各框架前端
> （Fcitx5 见 [linux-fcitx5.md](linux-fcitx5.md)，IBus 见 `apps/linux/ibus`），那一版的 deb / rpm / flatpak 打包也没有进 main。
> 它的 D-Bus 层与按键翻译移植进了 `apps/linux/ibus`，改成连 Server 的瘦客户端。留着这一页是因为**坑与验收数字仍然有效**：
> 下面五个坑（总线地址 / Shift+数字 / GVariant 嵌套 / 把 `/` 当仓库根 / 锁毒化）与 IBus 面板的限制，
> 换成瘦客户端接 Server 之后一样会碰到。
