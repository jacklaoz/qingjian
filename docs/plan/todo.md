...
- [ ] Linux（Phase 5，**只做 Wayland**，方案与分期见 [linux_plan.md](linux_plan.md)）：先 IBus + 系统面板打通（GNOME / KDE 的终态），
  再合入 `qingjian-render` 走原生 `input-method-v2` 自绘（只有 wlroots 系拿得到）；配置同步、跨平台词库
