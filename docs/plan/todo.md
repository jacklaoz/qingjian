...
- [~] 符号输入（2026-09-21）：`assets/symbol/symbol-zh.tsv` 按敲的输入码查、插第 2 位，
  `[general] extras` 四选一控制 emoji 与符号出不出；
  序号类另给每个数单独一个码（`yuanquanqi` → ⑦），解决「一次只出三个、第四个起够不着」。还差：
  - ~~`u` 前缀扩展成「码点 + 名字」统一入口~~：**不做**。`u` 后跟拼音已经是云端问字（本地没有候选、等答案），
    在那里加本地符号候选会改掉一个已发布功能的行为（首选变成符号，空格上屏的是符号而不是等答案）；
    而且原方案「首字符是数字或 `+` 才当码点」会把 `ufe0f` 这种字母开头的码点误判。它要解决的缺口改用上面的逐个码解了
  - 各壳装上符号表、设置界面加这个开关（macOS / Windows / Linux Server 各一个 PR）
  - 符号表覆盖面：99 个码，用起来缺什么再补；收录与命名的两条规矩见 [assets/symbol/README.md](../../assets/symbol/README.md)
  - 分类浏览（`vs` 进符号分类分页）：等真有人说找不到再做——现在的缺口（按名字找、序号够不着）都有了解法
- [ ] 日语 emoji 要等有日语输入模式
