...
- [~] 符号输入（2026-09-21 做了第一期）：`assets/symbol/symbol-zh.tsv` 69 个码，按敲的输入码查、插第 2 位，
  `[general] extras` 四选一控制 emoji 与符号出不出。还差：
  - `u` 前缀扩展成「码点 + 名字」统一入口（`ujiantou` → ➡️，首字符是数字或 `+` 当码点），现在只能靠候选里那几个
  - 设置界面还没有 `extras` 这一项（macOS 偏好设置 / Windows 设置程序），只能改配置文件
  - 符号表覆盖面：现在只收了「按名字找得到、键盘打不出来」的 69 条，用起来缺什么再补；
    收录与命名的两条规矩见 [assets/symbol/README.md](../../assets/symbol/README.md)
  - 分类浏览（`vs` 进符号分类分页）：等前两条落地、确认还找不到时再做
- [ ] 日语 emoji 要等有日语输入模式
