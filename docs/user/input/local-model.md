...
它会看光标前面的几十个字：同一段拼音在不同上下文里给的句子可能不同。应用不让读上下文时（部分终端、Electron 应用）只看这次会话里输入过的字；Linux 上一律只看这次会话里输入过的字。
...
「偏好设置 → 云服务」（Windows 与 Linux：「设置 → 云服务」）关闭「本地整句模型」。关闭后只用词库统计，与云联想互不影响。

Linux 上模型是单独的一个安装包（`qingjian-model`），未安装时没有模型可加载，此项开着也只用词库统计。
...
- **Linux**：`~/.local/share/qingjian/model/`
