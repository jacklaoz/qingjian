...
- `apps/linux`：`server`（Engine + Unix socket）+ `fcitx5`（框架事件、默认面板）+ `scripts`（用户级安装卸载）+ `packaging`（`build.sh` 在容器里打 deb / rpm，`smoke/run.sh` 在几个发行版的容器里装包自检，见 `docs/notes/linux-packaging.md`）；源码装的 Server 手动启动，见 `docs/notes/linux-fcitx5.md`。
