...
- `crates/qingjian-core`：引擎。`Engine` 是对外唯一门面，`Translator` / `Learner` 等 trait 在 `engine` 模块；拼音解析、纠错、候选、排序、整句、双拼、注音、英文模式、繁体输出、emoji 与符号都在这里。
...
- `crates/qingjian-platform`：平台层共用：`Config`（TOML 配置）、`extra_dictionaries`、`protocol` 线上类型（Windows Server ↔ DLL，以及 `protocol::linux` 的 Server ↔ 各前端）。
...
- `apps/linux`：`server`（Engine + Unix socket）+ `fcitx5`（框架事件、默认面板）+ `ibus`（第二个前端：IBus，按键 / 焦点 / 面板已通，对着真 daemon 验过）+ `scripts`（用户级安装卸载）+ `packaging`（`build.sh` 在容器里打 deb / rpm / flatpak，见 `docs/notes/linux-packaging.md`）；源码装的 Server 手动启动，见 `docs/notes/linux-fcitx5.md`。
