...
//!
//! **没有 ibus 就跳过**：CI 的 Linux runner 上没有 ibus-daemon，这个测试直接返回。
//! 本机怎么起一个私有 daemon 跑它，见 `apps/linux/README.md`。
...
impl Collected {
    ...
    fn summary(&self) -> String {
        ...
        let names: Vec<&str> = seen.iter().map(|(member, _)| member.as_str()).collect();
        format!("{} 条：{}", names.len(), names.join(", "))
    }
}
