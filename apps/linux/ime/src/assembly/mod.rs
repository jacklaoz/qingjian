//! 装配 Engine：壳里唯一知道具体 Translator / Learner 类型的地方。
//!
//! **这是第三份。** macOS 的 `host/init.rs`、Windows Server 的 `assembly/` 与这里做的是同一件事，
//! 而且本身平台无关。不能提到 `qingjian-platform` 共用：TSF DLL 依赖那个 crate，而架构约束定死
//! 「DLL 不能带 Engine 的依赖树」，把 learning / translate / lm 加进去就等于给 DLL 挂上整棵树。
//! 收口方案是 L3 的新 crate，见 `docs/plan/linux_plan.md`；在那之前三份各自维护，改一处要想想另两处。
...
/// 按清单装一个 Engine 出来。词库装不起来才算失败，其余数据缺了只记日志、少个功能。
pub fn assemble(spec: &AssemblySpec) -> Result<Engine, ShellError> {
...
fn user_dicts_dir(user_dir: Option<&Path>) -> Option<PathBuf> {
...
) -> Result<LayeredTranslator, ShellError> {
...
/// 几张 emoji 表合成一张；坏的跳过。
fn load_emoji(paths: &[PathBuf]) -> Option<EmojiTable> {
