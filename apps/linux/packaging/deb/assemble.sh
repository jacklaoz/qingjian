...
# - qingjian：Server、词库与本地整句模型、systemd 用户服务（build.sh 已摆好在 $stage/qingjian）
...
core="$stage/qingjian"
strip --strip-unneeded "$core/usr/bin/qingjian-linux-server"
...
normalize "$core"
control "$core" qingjian "$(shlibs "$core/usr/bin/qingjian-linux-server")" \
